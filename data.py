...
class Loader:
    ...
    def __init__(self, file_path: str, batch_size: int = 2, split_ratio: float = 0.9):
        ...
        pairs, self._num_classes = self._read()
        shuffle(pairs)
        self._train_set, self._test_set = self._split(pairs)

    def _read(self) -> Tuple[List[Tuple[np.ndarray]], int]:
        ...
        return list(zip(features_norm, one_hot_labels)), num_classes
...
def stack(batch: List[Tuple[np.ndarray]]) -> Tuple[np.ndarray]:
